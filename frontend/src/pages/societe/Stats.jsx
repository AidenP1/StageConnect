...
function SocieteStats() {
  ...
  const chartOptions = {
    ...
    plugins: {
      legend: { position: 'top', labels: { color: 'var(--text, #212529)' } },
      title: { display: true, text: t('stats.chartTitle'), color: 'var(--text, #212529)' },
    },
    scales: {
      x: { ticks: { color: 'var(--text-muted, #6c757d)' }, grid: { color: 'var(--border, #dee2e6)' } },
      y: { beginAtZero: true, ticks: { color: 'var(--text-muted, #6c757d)' }, grid: { color: 'var(--border, #dee2e6)' } },
    },
  };
