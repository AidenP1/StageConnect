...
import { getStagesRecommandeés } from '../../api/stages';
...
function EtudiantDashboard() {
  ...
  const [recommandes, setRecommandeés] = useState([]);
  ...
  useEffect(() => {
    const chargerDonnees = async () => {
      ...
      try {
        const recData = await getStagesRecommandeés();
        setRecommandeés(recData);
      } catch (err) {
        // erreur silencieuse
      } finally {
        setChargement(false);
        ...
        {recommandes.length > 0 && (
          <div className="card-custom p-4 mb-4">
            <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              <Link to="/etudiant/stages" className="btn btn-sm btn-outline-primary">
                {t('dashboard.viewAll')}
              </Link>
            </div>
            <div className="row g-3">
              {recommandes.map((stage) => (
