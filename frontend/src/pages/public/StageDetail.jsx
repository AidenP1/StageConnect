...
function StageDetail() {
  ...
  // vérifier si dans les favoris et si déjà postulé
  useEffect(() => {
    if (utilisateur && utilisateur.role === 'etudiant') {
      const verifier = async () => {
        try {
          const favoris = await getMesFavoris();
          setEstFavori(favoris.some((f) => f.stage_id === parseInt(id)));
        } catch (err) {}
        try {
          const candidatures = await getMesCandidatures();
          setDejaPostule(candidatures.some((c) => c.stage_id === parseInt(id)));
        } catch (err) {}
      };
      verifier();
    }
  ...
  const envoyerCandidature = async () => {
    setEnvoi(true);
    try {
      await postuler(id, fichierLettre);
      setDejaPostule(true);
  ...
  return (
    <div className="public-page">
      <div className="container py-4 flex-grow-1">
          ...
          <div className="row g-4">
            ...
            <div className="col-lg-4">
              <div className="card-custom p-4 sticky-sidebar">
                ...
                {messageSuccess && (
                  <div className="alert alert-success mb-3 text-center" style={{ fontSize: '0.9rem' }}>
                ...
                <div className="d-grid gap-2">
                  {(utilisateur?.role === 'societe' || utilisateur?.role === 'admin') ? null : dejaPostule ? (
                    <button className="btn btn-outline-success" disabled>
                      <i className="fas fa-check me-2"></i>
                      {t('stages.alreadyApplied')}
                    </button>
                  ) : (
                    <button
        ...
        {/* modal postuler */}
        {modalPostuler && (
          <div className="modal-overlay" onClick={() => { setModalPostuler(false); setFichierLettre(null); }}>
            <div className="modal-box card-custom" onClick={(e) => e.stopPropagation()}>
              <h5 className="mb-1">{t('stageDetail.coverLetter')}</h5>
              <p className="text-muted mb-3" style={{ fontSize: '0.85rem' }}>{stage.title}</p>
              <div className="mb-3">
              ...
              <div className="d-flex gap-2 justify-content-end">
                <button className="btn btn-outline-secondary" onClick={() => { setModalPostuler(false); setFichierLettre(null); }}>
