...
function StagesPublic() {
  ...
  // charger les candidatures de l'étudiant pour marquer "déjà postulé"
  useEffect(() => {
    if (utilisateur?.role === 'etudiant') {
      getMesCandidatures()
        .then((data) => setCandidaturesIds(data.map((c) => c.stage_id)))
        .catch(() => {});
    }
  }, [utilisateur]);
              ...
              <div className="row g-3">
                {stagesPage.map((stage) => (
                  <div key={stage.id} className="col-md-6 col-lg-4">
                    <StageCard stage={stage} dejaPostule={candidaturesIds.includes(stage.id)} />
                  </div>
                ))}
              </div>
