...
class SocieteController extends Controller
{
    ...
    // toutes les candidatures récentes de la société (toutes offres confondues)
    public function candidaturesRecentes(Request $request)
    {
        $societeId = $request->user()->id;
        $stageIds  = Stage::where('societe_id', $societeId)->pluck('id');

        $candidatures = Candidature::whereIn('stage_id', $stageIds)
            ->with(['etudiant.etudiantProfile', 'stage'])
