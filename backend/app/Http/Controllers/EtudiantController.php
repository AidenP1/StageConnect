...
class EtudiantController extends Controller
{
    ...
    // algorithme de recommandation — matching par compétences
    public function stagesRecommandeés(Request $request)
    {
        $profil = $request->user()->etudiantProfile;
        $studentSkills = $profil && $profil->skills
            ? array_map('trim', explode(',', strtolower($profil->skills)))
            : [];
        ...
        // calculer le score de correspondance pour chaque stage
        $scored = $stages->map(function ($stage) use ($studentSkills) {
            $stageSkills = $stage->skills_required
                ? array_map('trim', explode(',', strtolower($stage->skills_required)))
                : [];

            $matches = 0;
            foreach ($studentSkills as $sk) {
                foreach ($stageSkills as $req) {
            ...
            $score = count($stageSkills) > 0
                ? round(($matches / max(count($stageSkills), 1)) * 100)
                : 0;

            $stage->match_score = $score;
            return $stage;
        })
